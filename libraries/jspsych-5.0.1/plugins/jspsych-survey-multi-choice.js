 ...
 */
/*
  Customized by Stefan Uddenberg — 04/05/2016

  FEATURE LOG:
    - Can specify response question names (makes text validation possible) — trial.question_names
 */
...
jsPsych.plugins['survey-multi-choice'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    trial.required_asterisk = typeof trial.required_asterisk == 'undefined' ? true : trial.required_asterisk;
    trial.horizontal = typeof trial.required == 'undefined' ? false : trial.horizontal;

    if (typeof trial.question_names == 'undefined') {
      trial.question_names = [];
      for (var i = 0; i < trial.questions.length; i++) {
        trial.question_names.push(" ");
      }
    }
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if (trial.required && trial.required[i]) {
        ...
        if (trial.required_asterisk==true) {
    ...
    $trial_form.submit(function(event) {
      ...
      $("div." + plugin_id_name + "-question").each(function(index) {
        if (trial.question_names[index]!="") {
          var id = trial.question_names[index];
        } else {
          var id = "Q" + index;
        };
